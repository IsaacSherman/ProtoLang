...
| Checked integer arithmetic | Yes | Yes | — | Terminates with exit code 70 on overflow. Selected by `Arithmetic/Overflow` ([10.4](./§10-Numeric Semantics.md#104-compile-time-policy)). |
| Saturating integer arithmetic | Yes | Yes | — | Clamps to the exceeded bound. Selected by `Arithmetic/Overflow` ([10.4](./§10-Numeric Semantics.md#104-compile-time-policy)). |
| Compile-time policy file | Yes | Yes | — | `protolang.config.xml`, found by walking up from the source ([10.4](./§10-Numeric Semantics.md#104-compile-time-policy)). |
| Checked division (`on_zero`) | Yes | Yes | — | Runtime zero check in both; see 10.2.1. |
| `on_zero fail` | Yes | Yes | — | `Environment.Exit(70)` / `std::_Exit(70)`, after a diagnostic on stderr ([10.2.1](./§10-Numeric Semantics.md#1021-the-on_zero-clause)). |
...
| Conditionals and `while` | Yes | Yes | — | `if` / `else if` / `else`, `while`, `break`, `continue` ([15](./§15-Control Flow.md#15-control-flow)). |
| Field presence (`has`) | Yes | Yes | — | `x != null` or `HasX` in C#; `has_x()` in C++ ([8.4](./§8-Type System.md#84-nullability-and-presence)). |
...
| Proto2 presence | Yes | Yes | — | Via `FieldDescriptor.HasPresence`; no syntax-version branch ([21.3](./§21-Interoperability With Protobuf.md#213-protobuf-editions-and-syntax-versions)). |
| Proto3 optional | Yes | Yes | — | Same mechanism. |
| Editions | Yes | Yes | — | Same mechanism; presence is a resolved feature ([21.3](./§21-Interoperability With Protobuf.md#213-protobuf-editions-and-syntax-versions)). |
