...
- That source info is answered, not merely kept. Given a message, enum, field, or enum value
  ...
  built without source info, a file that cannot be read, and a schema `protoc` resolved from
  descriptors compiled into itself all give an answer with nothing in it -- which is a different
  answer from none at all, none meaning the compilation does not hold that file. A range is reported
  only where the file it points into could be read, because a range is measured against text;
  documentation does not depend on the file, so a schema may be documented and nowhere on disk. The
  positions are the compiler's own -- 1-based lines and columns counting UTF-16 code units -- rather
  than the byte counts `protoc` reports, whose columns advance by the width of a character in bytes
  and jump a tab to the next multiple of eight, and so name a different character on any line
  holding a tab or non-ASCII text.
