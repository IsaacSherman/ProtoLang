...
- That source info is answered, not merely kept. Given a message, enum, field, or enum value
  ...
  points confidently at the wrong characters; documentation does not depend on the file at all, so a
  schema may be documented and nowhere on disk, or documented and since edited. The positions are the
